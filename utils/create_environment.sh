...
function create_env_and_configs {
    ...
    if [ "$ENV_TYPE" == "develop" ]; then
        ...
        kubectl -n $KUBERNETES_NAMESPACE create secret generic daedalus-core-database --from-literal=MYSQL_USER="daedalus" --from-literal=MYSQL_PASSWORD="$DAEDALUS_CORE_DEVELOP_NEW_USER_PASSWORD" --from-literal=MYSQL_HOST="percona-server-develop" --from-literal=MYSQL_PORT="3306" --from-literal=MYSQL_DATABASE="daedalus_core_realms" --from-literal=MYSQL_CONECTION_RETRIES="5" --from-literal=MYSQL_CONECTION_TIMEOUT="5"
